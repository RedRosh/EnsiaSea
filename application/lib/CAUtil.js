...
exports.enrollAdmin = async (caClient, wallet, orgMspId) => {
	try {
		...
		return {
			success : true,
			message : "Successfully enrolled admin user and imported it into the wallet"
		};
	} catch (error) {
		return {
			success : false,
			message : `Failed to enroll admin user : ${error}`
		};
	}
};
...
exports.registerAndEnrollUser = async (caClient, wallet, orgMspId, userId, affiliation) => {
	try {
		...
		return {
			success : true,
			message : `Successfully registered and enrolled user ${userId} and imported it into the wallet`
		};
	} catch (error) {
		return {
			success : false,
